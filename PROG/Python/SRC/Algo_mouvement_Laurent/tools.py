...
def intersect(A, B):
    ...
    if inter_type == 1: #Cercle avec segment/droite
        lambdas = []
        if objectB[0] == 'S':
            u = (objectB[2]-objectB[1])/norm(objectB[2]-objectB[1])
        else:
            u = objectB[2]/norm(objectB[2])
        delta = (np.dot(objectB[1]-objectA[1], u)**2) - ((norm(objectB[1]-objectA[1]))**2 - objectA[2]**2)
        if delta == 0 :
            lambdas += [-np.dot(objectB[1]-objectA[1], u)]
        elif delta > 0:
            lambdas += [(-np.dot(objectB[1]-objectA[1], u)-sqrt(delta))]
            lambdas += [(-np.dot(objectB[1]-objectA[1], u)+sqrt(delta))]
        inters=[]
        for l in lambdas:
            if objectB[0]=='L' or (l>0 and l<norm(objectB[1]-objectB[2])):
                inters += [objectB[1]+l*u]
        return inters
    if inter_type == 2: #Cercle avec cercle
        D = norm(objectA[1] - objectB[1])
        if D == 0:
            return []
        ...
        if inter_type == 3:
            if objectA[0] == 'S':
                u1 = (objectA[2]-objectA[1])/norm(objectA[2]-objectA[1])
            else:
                u1 = objectA[2]
            if objectB[0] == 'S':
                u2 = (objectB[2]-objectB[1])/norm(objectB[2]-objectB[1])
            else:
                u2 = objectB[2]
            ...
            if objectA[0]=='S' and (norm(objectA[2]-objectA[1])<l1 or l1<0):
                return []
            l2 = np.dot(objectA[1]-objectB[1], v1)/np.dot(u2, v1)
            if objectB[0]=='S' and (norm(objectB[2]-objectB[1])<l2 or l2<0):
                return []
